...
const navigation = {
  main: [
    ...
    { name: "Call To Book", href: "tel:3373486937" },
  ],
  social: [
    {
      name: "Facebook",
  ...
  return (
    <footer className="bg-white">
      <div className="mx-auto max-w-[100rem] overflow-hidden sm:px-6 py-8 sm:py-16 lg:px-8">
        ...
        >
          {navigation.main.map((item) => (
            <div key={item.name} className="pb-6">
              <a
                href={item.href}
                className="text-sm leading-6 text-gray-600 hover:text-[#653033]"
              >
                {item.name}
              </a>
            </div>
        ...
        <div className="mt-10 flex sm:justify-center px-3 sm:px-0  space-x-10">
          {navigation.social.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-gray-400 hover:text-[#653033]"
            >
        ...
        <p className="mt-10 border-t sm:border-hidden pt-6 sm:pt-0 text-center text-sm leading-5 text-gray-500">
          &copy; 2023 <span className="font-semibold">BDA Inspections</span>.
          All rights reserved.
          <p>
            Trec:{" "}
            <a
